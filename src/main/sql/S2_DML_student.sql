...
-- [TEST]
CREATE OR REPLACE VIEW s2_1 AS -- [TEST]
...
--[TEST]
CREATE OR REPLACE VIEW s2_2 AS -- [TEST]
SELECT *
FROM medewerkers
...
-- [TEST]
CREATE OR REPLACE VIEW s2_3 AS -- [TEST]
SELECT cursus, begindatum
FROM uitvoeringen
WHERE locatie ILIKE 'UTRECHT'
   OR locatie ILIKE 'MAASTRICHT';
...
-- [TEST]
CREATE OR REPLACE VIEW s2_4 AS -- [TEST]
SELECT naam, voorl
FROM medewerkers
WHERE NOT (voorl ILIKE 'R' AND naam ILIKE 'Jansen');
...
SELECT medewerkers.mnr, 'S02', DATE '2021-03-02', 'LEERDAM'
FROM medewerkers
WHERE medewerkers.naam ILIKE 'SMIT'
  AND medewerkers.voorl ILIKE 'N%'
...
VALUES (DEFAULT, 'STAGIAIR', 'WAAL', '7902')
ON CONFLICT DO NOTHING;
...
VALUES ('D&P', 'data persisteren op een juiste manier', 'ALG', 6)
ON CONFLICT DO NOTHING;

INSERT INTO uitvoeringen (cursus, begindatum, docent, locatie)
SELECT 'D&P', DATE '2020-10-1', medewerkers.mnr, 'Leerdam'
FROM medewerkers
WHERE medewerkers.functie ILIKE 'TRAINER'
ORDER BY gbdatum DESC
LIMIT 1
ON CONFLICT DO NOTHING;

INSERT INTO uitvoeringen (cursus, begindatum, docent, locatie)
SELECT 'D&P', '2020-11-5', medewerkers.mnr, 'Leerdam'
FROM medewerkers
WHERE medewerkers.functie ILIKE 'TRAINER'
ORDER BY gbdatum
LIMIT 1
...
VALUES (7654, 'D&P', DATE '2020-10-1', null),
       (7499, 'D&P', DATE '2020-10-1', null),
       (7844, 'D&P', DATE '2020-11-5', null),
       (7521, 'D&P', DATE '2020-11-5', null)
ON CONFLICT DO NOTHING;
-- [TEST]
...
WHERE afd = (SELECT anr FROM afdelingen WHERE naam ILIKE 'VERKOOP');

UPDATE medewerkers
SET maandsal = maandsal * 1.07
WHERE mnr = (SELECT chef WHERE afd = (SELECT anr FROM afdelingen WHERE naam ILIKE 'VERKOOP'));
...
-- DELETE
-- FROM medewerkers
-- WHERE (naam ILIKE 'Martens' AND afd = (SELECT afd FROM afdelingen WHERE naam ILIKE 'VERKOOP'));

-- DELETE
-- FROM medewerkers
-- WHERE (naam ILIKE 'Alders' AND afd = (SELECT afd FROM afdelingen WHERE naam ILIKE 'VERKOOP'));
-- Omdat hij nog referenties heeft naar andere tabellen. Een alter table met ON DELETE CASCADE zou hier de uitkomst zijn.
...
INSERT INTO medewerkers (mnr, naam, voorl, functie, chef, gbdatum, maandsal, comm, afd, geslacht)
VALUES (9873, 'DOL', 'M', 'FINANCIEN', (SELECT mnr FROM medewerkers WHERE naam ILIKE 'DE KONING'), '2001-2-3', 2400,
        null, (SELECT anr FROM afdelingen WHERE naam ILIKE 'FINANCIEN'), 'M')
ON CONFLICT DO NOTHING;
INSERT INTO afdelingen (anr, naam, locatie, hoofd)
VALUES (DEFAULT, 'FINANCIEN', 'WAAL',
        (SELECT mnr FROM medewerkers WHERE voorl ILIKE 'M' AND medewerkers.naam ILIKE 'DOL'))
ON CONFLICT DO NOTHING;
