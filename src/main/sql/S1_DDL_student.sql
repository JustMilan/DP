...
-- F[23514] ERROR: new row for relation "medewerkers" violates check constraint "m_geslacht_chk" Detail:-- Failing row contains (7369, SMIT, N, TRAINER, 7902, 1985-12-17, 800.00, null, 20, D
...
VALUES (8000, 'DONK', 'A', 'ONDERZOEK', 7839, '1978-10-10', 3800, null, null, 'M');
INSERT INTO afdelingen
VALUES (50, 'ONDERZOEK', 'ZWOLLE', 8000);
UPDATE medewerkers
SET afd = 50
WHERE functie LIKE 'ONDERZOEK';
...
CREATE SEQUENCE afdeling_afdelingnmr_seq INCREMENT 10 START 60 MINVALUE 0 CACHE 10;
ALTER TABLE afdelingen
    ALTER COLUMN anr SET DEFAULT nextval('afdeling_afdelingnmr_seq');
INSERT INTO afdelingen
VALUES (DEFAULT, 'PROGRAMMEREN', 'ROTTERDAM', null),
       (DEFAULT, 'SERVICE', 'AMSTERDAM', null);
ALTER TABLE afdelingen
...
ALTER TABLE adressen
    ADD CONSTRAINT a_pstcd_chk CHECK (postcode ~ ('[0-9][0-9][0-9][0-9][A-Z][A-Z]'));
ALTER TABLE adressen
    ADD CONSTRAINT a_bgndtm_chk CHECK ( ingangsdatum < CURRENT_DATE );
ALTER TABLE adressen
    ADD CONSTRAINT a_einddtm_chk CHECK ( einddatum > CURRENT_DATE AND einddatum > ingangsdatum );
...
ALTER TABLE medewerkers
    DROP COLUMN IF EXISTS geslacht CASCADE;
