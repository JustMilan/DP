...
CREATE OR REPLACE VIEW s3_1 AS -- [TEST]
SELECT cursus, begindatum, lengte, m.naam
FROM uitvoeringen u
...
CREATE OR REPLACE VIEW s3_2 AS -- [TEST]
...
CREATE OR REPLACE VIEW s3_3 AS -- [TEST]
SELECT a.naam afdeling, m.naam hoofd
FROM afdelingen a
         INNER JOIN medewerkers m ON m.mnr = a.hoofd;

-- S3.4.
-- Geef de namen van alle medewerkers, de naam van hun afdeling (`afdeling`)
-- en de bijbehorende locatie.
DROP VIEW IF EXISTS s3_4;
CREATE OR REPLACE VIEW s3_4 AS -- [TEST]
...
CREATE OR REPLACE VIEW s3_5 AS -- [TEST]
SELECT m.naam
FROM medewerkers m
         INNER JOIN inschrijvingen i on m.mnr = i.cursist and i.cursus = 'S02' and begindatum = '2019-04-12';

-- S3.6.
-- Geef de namen van alle medewerkers en hun toelage.
DROP VIEW IF EXISTS s3_6;
CREATE OR REPLACE VIEW s3_6 AS -- [TEST]
SELECT m.naam, s.toelage
FROM medewerkers m
         INNER JOIN schalen s ON s.ondergrens <= m.maandsal and m.maandsal <= s.bovengrens;
