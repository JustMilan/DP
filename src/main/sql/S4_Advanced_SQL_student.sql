...
-- (met [TEST] erachter) te activeren (haal hiervoor de commentaartekenst
...
CREATE OR REPLACE VIEW s4_1 AS -- [TEST]
...
CREATE OR REPLACE VIEW s4_2 AS -- [TEST]
SELECT naam
FROM medewerkers
WHERE naam LIKE '% %';
...
SELECT u.cursus code, u.begindatum, count(i.cursist) aantal_inschrijvingen
FROM uitvoeringen u
         INNER JOIN inschrijvingen i ON u.cursus = i.cursus AND u.begindatum = i.begindatum AND
                                        u.begindatum BETWEEN '2019-01-01' AND '2020-01-01'
...
CREATE OR REPLACE VIEW s4_4 AS -- [TEST]
SELECT m.mnr, i.cursus
FROM medewerkers m
...
SELECT cursus, COUNT(cursus) aantal
FROM uitvoeringen
GROUP BY cursus;
...
CREATE OR REPLACE VIEW s4_6 AS -- [TEST]
SELECT DATE_PART('year', max(gbdatum)) - DATE_PART('year', min(gbdatum)) verschil, avg(AGE(gbdatum))
FROM medewerkers m;
...
CREATE OR REPLACE VIEW s4_7 AS -- [TEST]
SELECT count(m.mnr)               aantal_medewerkers,
       sum(m.comm) / count(m.mnr) commissie_medewerkers,
       avg(DISTINCT m.comm)       commissie_verkopers
FROM medewerkers m;
